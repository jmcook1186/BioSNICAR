...
function [alb_slr,albedo_spectral] = SNICAR_function_APP(BND_TYP,APRX_TYP,DIRECT,DELTA,coszen, R_sfc, ...
    dz, rho_snw, rds_snw,BC1,BC2,DST1,DST2,DST3,DST4,ASH1,BIO1,BIO2,BIO3,BIO4,BIO5,BIO6,BIO7,HM,SAND)


nbr_lyr  = length(dz);  % number of snow layers
nbr_aer = 17;

% PLACE x TO DETERMINE LOCATION AND TYPE OF ALGAL CELL
% PLACE y TO DETERMINE LOCATION AND TYPE OF MINERAL DUST

% PARTICLE MASS MIXING RATIOS (units: ng(species)/g(ice), or ppb)
mss_cnc_sot1(1:nbr_lyr)  = [BC1,0,0,0,0];  % uncoated black carbon
mss_cnc_sot2(1:nbr_lyr)  = [BC2,0,0,0,0];    % coated black carbon
mss_cnc_dst1(1:nbr_lyr)  = [DST1,0,0,0,0];    % dust species 1
mss_cnc_dst2(1:nbr_lyr)  = [DST2,0,0,0,0];    % dust species 2
mss_cnc_dst3(1:nbr_lyr)  = [DST3,0,0,0,0];    % dust species 3
mss_cnc_dst4(1:nbr_lyr)  = [DST4,0,0,0,0];    % dust species 4
mss_cnc_ash1(1:nbr_lyr)  = [ASH1,0,0,0,0];    % volcanic ash species 1
mss_cnc_bio1(1:nbr_lyr)  = [BIO1,0,0,0,0];    % Biological impurity species 1
mss_cnc_bio2(1:nbr_lyr)  = [BIO2,0,0,0,0];    % Biological impurity species 2
mss_cnc_bio3(1:nbr_lyr)  = [BIO3,0,0,0,0];    % Biological impurity species 3
mss_cnc_bio4(1:nbr_lyr)  = [BIO4,0,0,0,0];    % Biological impurity species 4
mss_cnc_bio5(1:nbr_lyr)  = [BIO5,0,0,0,0];    % Biological impurity species 5
mss_cnc_bio6(1:nbr_lyr)  = [BIO6,0,0,0,0];    % Biological impurity species 6
mss_cnc_bio7(1:nbr_lyr)  = [0,0,0,0,0];    % Biological impurity species 6
mss_cnc_Real_Bio1(1:nbr_lyr) = [BIO7,0,0,0,0]; % Algal cell with measured phenol
mss_cnc_water1(1:nbr_lyr) = [0,0,0,0,0];   % Water, 2 mm spheres
mss_cnc_hematite(1:nbr_lyr) = [HM,0,0,0,0];   % Water, 2 mm spheres
mss_cnc_mixed_sand(1:nbr_lyr) = [SAND,0,0,0,0];
